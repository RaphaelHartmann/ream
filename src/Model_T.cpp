...
double Model_T::approx_dt(double* phi, double dt_scale) const {
...
/* method for PDF */
int Model_T::pdf(double *RsumlogPDF, double *RPDFlow, double *RPDFupp, double *RlogPDFlow, double *RlogPDFupp, std::vector<double> rtl, std::vector<double> rtu, double *phi) const {
  ...
  double sigma_n[N_deps_max], sigma_np1[N_deps_max]; /* diffusion rate at time steps n and n plus 1 */
  double sigma2_n[N_deps_max], sigma2_np1[N_deps_max]; /* squared diffusion rate at time steps n and n plus 1 */
  ...
  for (ii = 0; ii < N_dt - 1; ii++) {
    ...
    for (jj = 1; jj < N_deps-1; jj++) {
      ...
      AA[jj] = alpha_np1*beta_np1 - alpha_np1*v_np1 - gamma_np1*sigma2_np1[jj-1];
      BB[jj] = 1.0 + 2.0*gamma_np1*sigma2_np1[jj];
      CC[jj] = -alpha_np1*beta_np1 + alpha_np1*v_np1 - gamma_np1*sigma2_np1[jj+1];

      DD[jj] = -alpha_n*beta_n + alpha_n*v_n + gamma_n*sigma2_n[jj-1];
      EE[jj] = 1.0 - 2.0*gamma_n*sigma2_n[jj];
      FF[jj] = alpha_n*beta_n - alpha_n*v_n + gamma_n*sigma2_n[jj+1];
    ...
    p_fpt[0][ii+1] = tt + tnd;
    p_fpt[1][ii+1] = fabs( 0.5 * sigma2_np1[N_deps-1] * (4.0*p_np1[N_deps-2] - p_np1[N_deps-3])/(2.0*deps*s_np1*s_ini) );
    p_fpt[2][ii+1] = fabs( 0.5 * sigma2_np1[N_deps-1] * (4.0*p_np1[1] - p_np1[2])/(2.0*deps*s_np1*s_ini) );
...
int Model_T::cdf(double *RsumlogCDF, double *RCDFlow, double *RCDFupp, double *RlogCDFlow, double *RlogCDFupp, std::vector<double> rtl, std::vector<double> rtu, double *phi) const {
...
int Model_T::rand(double *Rrt, double *phi) const {
...
int Model_T::grid_pdf(double *Rrt, double *Rpdf_u, double *Rpdf_l, double *phi) const {
